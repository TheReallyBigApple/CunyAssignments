...
$$\hat{baby \ weight} \ = \ 123.05 -8.94 * smoker$$
...
*The average baby weight is  123.05 ounces if the mother is a non-smoker*
*and 114.11 ounces if the mother is a smoker*

(c) Is there a statistically significant relationship between the average birth weight and smoking?

*Yes, the t value is high and the p value is low. The null hypothesis should be rejected.*
...
$$\hat{days \ absent} \ = \ 18.93 -9.11 \ * \ NotAboriginal \ + \ 3.1 \ * \ Male \ + \ 2.15 \ * \ SlowLearner$$

(b) Interpret each one of the slopes in this context.


*Aborignals on average miss 9.11 more days. (All other things being equal)*
*Males on average miss 3.1 more days.(All other things being equal)* 
*Slow Learners on average miss 2.15 more days.(All other things being equal)*
...
sprintf("The residual is %.3f", actual-predicted)
...
$$R^2 \ = \ \frac{Var(residuals)}{Var(observations)}$$

```{r}

var_residuals<-240.57
var_observations<-264.17
n<-146
k<-3          # 3 variables each with 2 values

r_squared<-var_residuals/var_observations

adj_r_squared<-r_squared * (n-1)/(n-3-1)

sprintf("R squared = %.3f.      Adj R Squared = %.3f", r_squared, adj_r_squared)
...
*Ethnicity. The Adjusted R squared is very low meaning the predictor (ethnicity) doesnt influence the response (absenteeism) very much.*
...
*When the temperature fell below 60, the O Ring damage increased.*
...
*This model provides a meaningful probability of o-ring damage in the temperature range of 50 to 60, i.e. at 52 degrees :*

$$P(damage \ at \ 52 \ degrees) = 11.663 - .2162*52 = .74$$ 

(c) Write out the logistic model using the point estimates of the model parameters.

$$Y_i \ =  \ log(\frac{\hat{p}}{1-\hat{p}} ) \ = 11.663 -.2162*temperature$$

(d) Based on the model, do you think concerns regarding O-rings are justified? Explain.

*Yes. The low pvalues suggest a high level of confidence that as temperatures get lower and lower, risk of catastrophe increases.*
...
# notes 
# orings[,1] = temp  orings[,2] = damage
# the long collection is 5 zeros, a mix of 01 -0.07 , 3 zeros ???  its 23 numbers
...
temp<-c(51,53,55)
# temp<-c(59,61,63)
le_results<-11.663-(0.2162*temp)    # result of linear equation
exp_x<-exp(le_results)
p_hats<-round(exp_x/(1+exp_x),4)

results.df<-cbind(temp,p_hats)

knitr::kable(results.df, caption='Logistic Probability of Damage', "simple")
...
plot(temp,p_hats, main="Logistic Probability O-Ring Damage", xlab="Temperature", ylab=" P(damage)", pch=19, frame=T)
# lowess() adds more granular regression (curved line)
lines(lowess(temp,p_hats), col="red")
...
*The sample size was too small and I would think there are other factors and more scientific analysis needs to be done.*
